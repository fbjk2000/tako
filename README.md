...
- **Tasks** — Tasks v2 ships across four phases (Phase 1 → 3, May 2026):
  - **Pulse view** (Phase 1) — outcome-first cards bucketed into Suggested by TAKO · Overdue · Today · This Week · No due date, ranked by deal urgency, contact decay and external signals (not by raw due date). Multi-entity link chips (deal · company · contact · campaign · project) on every card; click a chip to jump to that entity. Inbound query-string filters from elsewhere in the app pre-scope the view (`?deal=…`, `?contact=…`, etc.). Click any task body to open the **Context Lens** right dock; edit from the pencil on the card or the Edit button in the dock.
...
Florian found six open tasks on /tasks that rendered nowhere, could not be opened, and therefore could never be given a due date. The only way to touch one was a `PUT /api/tasks/{id}`. Three separate causes, all now closed.
...
- **"No due date" is a bucket now.** Every Pulse section was derived from `due_date`, and an un-dated task was quietly filed under Today, where it claimed a deadline it did not have and diluted the ranking. It gets its own section, last, hidden when empty, with a deliberately quiet pill. TaskNav gained the matching filter and lost **Snoozed**, which nothing in the product could set and which rendered a blank page if it ever had a count.
...
- `Diag - task visibility` workflow reports, per org, every open task and which of the three surfaces would show it. A healthy org reports zero dead ends.
- 16 new tests.
